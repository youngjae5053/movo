...
type Tab = "profile" | "sessions" | "workouts";
...
export function MemberDetailView({ initialMember }: MemberDetailViewProps) {
  ...
  return (
    <AppShell title={member.name} showBackButton backFallback="/members">
      ...
      {/* 회원 헤더 카드 */}
      <section className="mb-4 overflow-hidden rounded-2xl border border-white/[0.07] bg-gradient-to-br from-[#17191f] to-[#111318] p-5 shadow-xl shadow-black/40">
        <div className="flex items-center gap-4">
          <div className="relative flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-emerald-500/30 to-emerald-700/10 text-2xl font-bold text-emerald-300 ring-1 ring-emerald-500/20 shadow-[0_0_20px_rgba(16,185,129,0.12)]">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-1 text-sm text-zinc-500">{member.goal}</p>
          </div>
        </div>
        {/* 빠른 액션 */}
        <div className="mt-4 flex gap-2">
          <button
            ...
            className="flex flex-1 items-center justify-center gap-1.5 rounded-xl bg-emerald-500/15 py-2.5 text-sm font-semibold text-emerald-400 ring-1 ring-emerald-500/25 transition-all hover:bg-emerald-500/25 active:scale-95"
          >
          ...
          <Link
            ...
            className="flex flex-1 items-center justify-center gap-1.5 rounded-xl bg-white/[0.04] py-2.5 text-sm font-semibold text-zinc-400 ring-1 ring-white/[0.07] transition-all hover:bg-white/[0.08] active:scale-95"
          >
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
      ...
      <div className="mb-5 flex gap-1 rounded-xl bg-[#111318] p-1 ring-1 ring-white/[0.06]">
        {TABS.map((tab) => (
          <button
            ...
            className={[
              "flex-1 rounded-lg py-2 text-xs font-semibold tracking-wide transition-all",
              activeTab === tab.id
                ? "bg-emerald-500/20 text-emerald-400 shadow-[inset_0_1px_0_rgba(52,211,153,0.1)]"
